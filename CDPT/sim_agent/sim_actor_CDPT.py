...
from CDPT.model.CDPT import CDPT
from CDPT.sim_agent.utils import *
from CDPT.model.model_utils import roll_out
from CDPT.sim_agent.guidance_metrics import *



class CDPT_test(CDPT):
    def __init__(self, 
        cfg: dict,
        early_stop: int = 0,
        skip: int = 1,
        ...
        Args:
            ...
            guide_mode (str, optional): Guidance mode. Defaults to 'default'.
        """
        super().__init__(cfg)
        ...
        if guide_mode == 'default':
            self.guidance_func = self.guidance
        elif guide_mode == 'ctg':
    ...
    def inference_predictor(self, batch) -> Dict[str, torch.Tensor]:
        ...
        goal_outputs, _ = self.forward_predictor(encoder_outputs)
    ...
    def ctg_guidance(self, x_t: torch.Tensor, c: dict, t: int, **kwargs) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, Dict[str, np.ndarray]]:
        ...
        # Denoise and step
        denoiser_output, _ = self.forward_denoiser(
            encoder_outputs = c,
        ...
        with torch.enable_grad():
            ...
            for _ in range(self.guidance_iter):
                ...
                noised_trajs = roll_out(currnet_states, noised_actions,
    ...
    def guidance(self, x_t: torch.Tensor, c: dict, t: int, **kwargs) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, Dict[str, np.ndarray]]:
        """
        ...
        for _ in range(self.guidance_iter):
            with torch.enable_grad():
                ...
                guidance_denoiser_output, _ = self.forward_denoiser(
                    encoder_outputs = c,
    ...
    def ibr_guidance(
        self, x_t: torch.Tensor, c: dict, t: int,
        ego_idx: int, adv_idx: int, other_idx: list = None,
        ego_iter: int = 5, adv_iter = 5, 
        t_react: int = 81,
        ...
        for iter_round in range(self.guidance_iter):
            for iter in range(ego_iter+adv_iter):
                with torch.enable_grad():
                    mu = mu.detach()
                    mu.requires_grad_()
                    if (iter<adv_iter and adv_use_ctg) or (iter>=adv_iter and ego_use_ctg):
                        method = 'ctg'
                        noised_actions = self.unnormalize_actions(mu)
                        currnet_states = c['agents'][:, :self._agents_len, -1]
                        traj_pred = roll_out(currnet_states, noised_actions,
                            action_len=self.denoiser._action_len, global_frame=True)
                    ...
                    if iter<adv_iter:
                        ...
                        pursue_reward = torch.max(pursue_reward[:, 0, :, 1], dim=-1)[0] # [B, T] 
                        ...
                        onroad_reward = torch.mean(onroad_reward, dim=-1) # [B]
                        ...
                        print(method, ' Adv round: ', iter_round, ', step: ', iter, 
                                'pursue reward: ',round(pursue_reward.mean().item(),3),
                                'onroad_reward: ', round(onroad_reward.mean().item(),3)
                            )
                    else:
                        ...
                        evasion_reward[:, adv_i, :t_react, :] = 100 # adv Ignore collision before t_react
                        evasion_reward = evasion_reward.reshape(*evasion_reward.shape[:2], -1) # [B, A, T*A]
                        evasion_reward_min = torch.min(evasion_reward, dim=-1)[0] # [B, A]
                        ...
                        onroad_reward = torch.mean(onroad_reward, dim=-1) # [B, A]
                        ...
                        print(
                            method, ' ego round: ', iter_round, ', step: ', iter-adv_iter, 
                            'ego evasion reward: ',
                            round(evasion_reward_min[0,ego_i].item()+offset ,3),
                            'adv evasion reward: ',
                            round(evasion_reward_min[0,adv_i].item()+offset,3),
                            'onroad reward: ',
