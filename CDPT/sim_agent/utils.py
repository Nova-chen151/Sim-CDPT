...
from CDPT.waymax_visualization import utils
from CDPT.waymax_visualization import color
from CDPT.waymax_visualization import viz as visualization
...
def plot_simulator_state(
    ...
    visualization.plot_trajectory(
        ...
        is_ego=ego_idx, is_adv=adv_idx
    )  # pytype: disable=wrong-arg-types  # jax-ndarray

    # 2. Plots road graph elements.
    visualization.plot_roadgraph_points(ax, state.roadgraph_points, verbose=False)
    ...
    if show_goals:
        ...
        for goal in ego_goal:
            ...
            ax.plot(goal[ts, 0], goal[ts, 1], marker='^', color=color.COLOR_DICT['ego'], ms=6, alpha=alpha)
            i = i + 1

        ego_goal = chosen_goals[0]
        ax.plot(ego_goal[ts, 0], ego_goal[ts, 1], marker='X', color=color.COLOR_DICT['ego'], ms=6, alpha=1.0)
        ...
        if adv_idx is not None:
            ...
            for goal in adv_goal:
                ...
                ax.plot(goal[ts, 0], goal[ts, 1], marker='^', color=color.COLOR_DICT['adv'], ms=6, alpha=alpha)
                i = i + 1

            adv_goal = chosen_goals[1]
            ax.plot(adv_goal[ts, 0], adv_goal[ts, 1], marker='X', color=color.COLOR_DICT['adv'], ms=6, alpha=1.0)
    ...
    tt = ax.text(origin_x - 0.9*50, origin_y + 0.9*48, f"t={t:.1f} s", fontsize=18, zorder = 100)
