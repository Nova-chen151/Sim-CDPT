...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        encodings, attention_values, encodings_values = self.transformer_encoder(relations, encoded_agents, encoded_map_lanes, encoded_traffic_lights,
                                             agents_mask, maps_mask, traffic_lights_mask)
        
        encoder_outputs['encodings'] = encodings
        encoder_outputs['attention_values'] = attention_values  
        encoder_outputs['encodings_values'] = encodings_values 
...
class SelfTransformer(nn.Module):
    ...
    def forward(self, inputs, relations, mask=None):
        attention_output, attention_value = self.qc_attention(inputs, relations, mask)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, encoded_relations, encoded_trajs, encoded_polylines, encoded_traffic_lights,
        ...
        for layer in self.layers:
            encodings, attention_value = layer(encodings, encoded_relations, attention_mask)
            attention_values.append(attention_value)  
            encoding_values.append(encodings)  

        return encodings, attention_values, encoding_values
