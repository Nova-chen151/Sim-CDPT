...
# utils
from CDPT.data.dataset import WaymaxTestDataset
from CDPT.model.utils import set_seed
from CDPT.sim_agent.sim_actor_CDPT import CDPT_test, sample_to_action
from CDPT.waymax_visualization.plotting import plot_state
...
from CDPT.sim_agent.waymax_metrics import OffroadMetric, WrongWayMetric
from CDPT.sim_agent.waymax_env import WaymaxEnvironment
from CDPT.data.waymax_utils import create_iter
...
dataset = WaymaxTestDataset(
    data_dir=None,
    anchor_path='./CDPT/data/cluster_64_center_dict.pkl',
...
def run_simulation(args):
    ## Load model
    CDPT = CDPT_test.load_for_inference(args.model_path, map_location=args.device)
    CDPT.reset_agent_length(N_SIM_AGENTS)
    CDPT.eval()
    set_seed(args.seed)  
    ...
    for scenario_id, scenario in data_iter:  
        ...
        is_controlled = np.array(scenario.object_metadata.is_valid[:N_SIM_AGENTS])
        active_vehicles = is_controlled & non_static[:len(is_controlled)]
            ...
            if i == 0:
                ...
                with torch.no_grad():
                    ...
                    if args.test_mode == 'diffusion':
                        pred = CDPT.sample_denoiser(batch)
                        pred_traj = pred['denoised_trajs'].cpu().numpy()[0]

                    elif args.test_mode == 'prior':
                        pred = CDPT.inference_predictor(batch)
